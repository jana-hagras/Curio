...
const ORDER_QUERY = `
  SELECT o.*, u.FName, u.LName,
    (SELECT TotalAmount    FROM Payment WHERE Order_id = o.Order_id LIMIT 1) AS TotalAmount,
    (SELECT PaymentMethod  FROM Payment WHERE Order_id = o.Order_id LIMIT 1) AS PaymentMethod,
    (SELECT Status         FROM Payment WHERE Order_id = o.Order_id LIMIT 1) AS PaymentStatus
  FROM \`Order\` o
