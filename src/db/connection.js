...
db.connect(err => {
    ...
    db.query("CREATE DATABASE IF NOT EXISTS CURIO", (err) => {
        if (err) throw err;
        console.log("Database CURIO checked/created");

        // Switch to the database
        db.query("USE CURIO", (err) => {
            if (err) throw err;
            console.log("Using CURIO database 🗄️");
            createAllTables(); // Initialize tables after selecting DB
        });
    });
});
...
const createAllTables = () => {
    ...
    // Sequential execution to respect Foreign Key hierarchy constraints
    ...
    .then(() => console.log("All tables created/verified successfully ✅"))
