...
import { FiCheck, FiEdit3, FiChevronDown, FiChevronRight, FiBriefcase, FiFileText, FiX, FiPrinter, FiTrash2, FiPlus, FiDollarSign, FiCalendar } from 'react-icons/fi';
              ...
              <div key={order.id} style={{ background: 'var(--surface-primary)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--surface-border)', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Buyer: {order.buyerName} • {formatDate(order.orderDate)}</p>
                  </div>
                </div>
                ...
                <div key={app.id} style={{ background: 'var(--surface-primary)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--surface-border)', overflow: 'hidden', transition: 'all 0.2s' }}>
                  ...
                  <div onClick={() => toggleExpand(app)}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div>
                        <p style={{ fontWeight: 600, fontSize: 15, marginBottom: 2 }}>Request #{app.request_id}</p>
                        ...
                        <div>
                          ...
                          {showAddForm === app.request_id && (
                            <div style={{ background: 'var(--surface-secondary)', padding: 16, borderRadius: 'var(--radius-md)', marginBottom: 16, border: '1px solid var(--surface-border)' }}>
                              <h5 style={{ fontSize: 13, fontWeight: 600, marginBottom: 12, color: 'var(--text-primary)' }}>New Milestone</h5>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                                <input value={addForm.title} onChange={e => setAddForm({ ...addForm, title: e.target.value })} style={inputStyle} placeholder="Milestone title *" />
                                <input value={addForm.escrowAmount} onChange={e => setAddForm({ ...addForm, escrowAmount: e.target.value })} style={inputStyle} placeholder="Escrow amount ($)" type="number" min="0" step="0.01" />
                              </div>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                                <input value={addForm.description} onChange={e => setAddForm({ ...addForm, description: e.target.value })} style={inputStyle} placeholder="Description" />
                                <input value={addForm.dueDate} onChange={e => setAddForm({ ...addForm, dueDate: e.target.value })} style={inputStyle} type="date" />
                              </div>
                              <div style={{ display: 'flex', gap: 8 }}>
                                <Button size="sm" onClick={() => handleAddMilestone(app.request_id)} loading={addingMilestone} icon={FiPlus}>Add</Button>
                                <Button size="sm" variant="ghost" onClick={() => { setShowAddForm(null); setAddForm({ title: '', description: '', escrowAmount: '', dueDate: '' }); }}>Cancel</Button>
                              </div>
                            </div>
                          )}

                          {reqMilestones.length === 0 ? (
                            <p style={{ fontSize: 14, color: 'var(--text-tertiary)', fontStyle: 'italic' }}>No milestones yet. Click "Add Milestone" to create one.</p>
                          ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative' }}>
                              {/* Vertical timeline line */}
                              <div style={{ position: 'absolute', left: 17, top: 18, bottom: 18, width: 2, background: 'var(--surface-border)', zIndex: 0 }} />
                                    ...
                                    {editingMilestone === m.id ? (
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                                          <input value={editForm.title} onChange={e => setEditForm({ ...editForm, title: e.target.value })} style={inputStyle} placeholder="Title" />
                                          <input value={editForm.escrowAmount} onChange={e => setEditForm({ ...editForm, escrowAmount: e.target.value })} style={inputStyle} placeholder="Escrow ($)" type="number" min="0" step="0.01" />
                                        </div>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                                          <input value={editForm.description} onChange={e => setEditForm({ ...editForm, description: e.target.value })} style={inputStyle} placeholder="Description" />
                                      ...
                                      <>
                                        ...
                                        {m.status !== 'Completed' && (
                                          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                                            ...
                                            <Button size="sm" onClick={() => setConfirmDialog({ open: true, milestoneId: m.id, requestId: app.request_id, type: 'milestone' })} icon={FiCheck}>Complete</Button>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 20, fontFamily: 'var(--font-body)', fontWeight: 600, marginBottom: 12 }}>
              {confirmDialog.type === 'delete-milestone' ? 'Delete Milestone' : 'Confirm Completion'}
            </h3>
            <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 24, lineHeight: 1.6 }}>
              {confirmDialog.type === 'delete-milestone'
                ? 'Are you sure you want to delete this milestone? This action cannot be undone and will recalculate escrow amounts.'
                : `Are you sure you want to mark this ${confirmDialog.type === 'order' ? 'order' : 'milestone'} as completed? This will finalize the status for both you and the buyer.`}
            </p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
              ...
              {confirmDialog.type === 'delete-milestone'
                ? <button onClick={handleDeleteMilestone} style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'var(--error)', color: '#fff', fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>Delete Milestone</button>
                : <Button onClick={handleDeleteMilestone}>Confirm Completion</Button>}
            </div>
          </div>
        </div>
      )}
