...
export default function MarketplacePage() {
  ...
  const handleAddToCart = (e, product) => {
    ...
    addItem(product);
    toast.success(
      `${product.item || product.itemName} ${t("common:nav.addedToCart") || "added to cart!"}`,
    );
  };
        ...
        <main className="peaceput-catalog">
          {loading ? (
            ...
            <EmptyState
              ...
              message={t("marketplace:noProductsDesc")}
            />
          ) : (
            <div className="peaceput-items-list">
              {filtered.map((product) => {
                ...
                return (
                  <div
                    key={product.id}
                    className="peaceput-item"
                    id={`product-item-${product.id}`}
                  >
                    <Link
                      to={`/marketplace/${product.id}`}
                      className="peaceput-item-link"
                    ...
                    >
                      ...
                      <div className="peaceput-item-details">
                        <h2 className="peaceput-item-title">
                        ...
                        <div className="peaceput-spec-table">
                          <div className="peaceput-spec-row">
                            <span className="spec-label">
                              {t("marketplace:spec.artisan")}
                            </span>
                            <span className="spec-value">
                              {product.artisanName ||
                                t("marketplace:masterArtisan")}
                            </span>
                          </div>
                          <div className="peaceput-spec-row">
                            <span className="spec-label">
                              {t("marketplace:spec.category")}
                            </span>
                            <span className="spec-value">
                              {t(
                        ...
                        <p className="peaceput-item-description">
                          {product.description}
                        </p>
                        ...
                        <div className="peaceput-item-actions">
                          {isBuyer && product.availQuantity > 0 && (
                            ...
                            >
                              ...
                              {t("marketplace:addToCart").toUpperCase()}
                            </button>
                          )}
                          {product.availQuantity === 0 && (
                            <span className="peaceput-sold-label">
                              {t("common:nav.adminPanel") === "Admin Panel"
                                ? "SOLD OUT"
                                : "نفذت الكمية"}
                            </span>
